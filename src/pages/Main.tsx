...
export default function MainPage() {
    const [restaurants, setRestaurants] = useState<RestaurantWithStats[] | null>(null);

    const [isCategoryModalOpen, setIsCategoryModalOpen] = useState(false);

    const [likedList, setLikedList] = useState<{ restaurant_id: number; liked: boolean }[]>([]);
    const [isLoading, setIsLoading] = useState<boolean>(true);

    const [searchParams, setSearchParams] = useSearchParams();

    const [searchValue, setSearchValue] = useState(searchParams.get("q") ?? "");
    const [selectedCategories, setSelectedCategories] = useState(searchParams.get("category") ? searchParams.get("category")!.split(",") : ["전체"]);
    const [sortType, setSortType] = useState<SortType>((searchParams.get("sort") as SortType) ?? "liked_count");

    const search = async () => {
        setIsLoading(true);

        // 기본 쿼리 구성
        let query = supabase.from("restaurants_with_stats").select("*").order(sortType, { ascending: false }).ilike("name", `%${searchValue}%`);

        // ✅ '전체'가 아닌 경우 category 필터링 추가
        if (!(selectedCategories.length === 1 && selectedCategories[0] === "전체")) {
                ...
                <div className="flex gap-4 items-center mb-4">
                    ...
                    >
                        {selectedCategories.length === 1 && selectedCategories[0] === "전체"
                            ? "카테고리 전체"
                            : `카테고리: ${selectedCategories.slice(0, 2).join(", ")}${selectedCategories.length > 2 ? ` 외 ${selectedCategories.length - 2}개` : ""}`}
                    </button>
                </div>
                {isCategoryModalOpen && (
                    <div className="fixed inset-0 bg-black/30 z-[1000] flex justify-center items-end" onClick={() => setIsCategoryModalOpen(false)}>
                        <CategoryModal selected={selectedCategories} onChange={setSelectedCategories} onClose={() => setIsCategoryModalOpen(false)} onClick={(e) => e.stopPropagation()} />
                    </div>
                )}
                {/* 결과 섹션 */}
                <div className="p-2">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-12">
                            <img src="/loading.gif" alt="로딩 중" className="w-32 h-32 object-contain mb-4" />
                        ...
                        <div className="flex flex-wrap gap-3">
                            {restaurants.map((item, idx) => (
                                <RestaurantCard
                                    ...
                                    isLiked={likedList.filter((liked) => liked.restaurant_id === item.id).length > 0}
                        ...
                        <div className="flex flex-col items-center justify-center text-center py-24 bg-[#fff2ed] rounded-lg mt-10">
                            <img src="/no_results.png" alt="검색 결과 없음" className="w-48 h-48 object-contain mb-6 opacity-70" />
                            <p className="text-2xl text-[#e4573d] font-jua">검색 결과가 없습니다 😢</p>
                        </div>
                    )}
                </div>
            </main>
...
function SortSelector({ onChange, value }: { onChange: (value: SortType) => void; value: SortType }) {
    return (
