...
function MyPage() {
    ...
    useEffect(() => {
        const fetchUserAndComments = async () => {
            ...
            const { data: user, error: userErr } = await supabase.from("users").select("*").eq("user_internal_id", uid).single();
            ...
            const { data: reviews, error: reviewErr } = await supabase.from("reviews").select("*, restaurants(place_name)").eq("user_id", user.user_internal_id);
    ...
    if (!userId) {
        return (
            <div className="w-[60vw] mx-auto mt-10 text-center space-y-4">
                ...
                <button className="mt-4 px-4 py-2 bg-black text-white rounded-md" onClick={() => navigate("/")}>
                    홈으로 이동
                </button>
            </div>
        );
    ...
    return (
        <div className="w-full max-w-3xl mx-auto min-h-screen mt-10 space-y-6 px-6">
            ...
            <div className="w-full flex justify-between items-start bg-white p-4 rounded-xl shadow-sm border">
                ...
                <Button variant={"ghost"} size={"icon"} onClick={() => navigate("/edit-profile", { state: { nickname, id: userId } })}>
                    <Settings />
                </Button>
            </div>
            <Separator />
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex justify-between items-center">
                    ...
                    {hasMore && (
                        <button className="text-sm text-neutral-500 hover:underline" onClick={() => setShowAll((prev) => !prev)}>
                            {toggleLabel}
                        </button>
                    )}
                </div>
