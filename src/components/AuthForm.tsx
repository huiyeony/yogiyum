...
export const AuthForm = ({ type, onSubmit }: Props) => {
  ...
  return (
    <div className="w-full max-w-sm mx-auto p-6 bg-red-100 shadow rounded-2xl">
      <h2 className="text-2xl font-bold mb-4 text-center font-['Gowun_Dodum']">
        {type === "login" ? "로그인" : "회원가입"}
      </h2>
      <form
        onSubmit={(e) => {
      ...
      >
        ...
        <Button
          type="submit"
          className="w-full font-['Gowun_Dodum'] bg-red-400 hover:bg-red-600 transition-colors duration-200"
        >
          {type === "login" ? "로그인" : "회원가입"}
        </Button>
      </form>
