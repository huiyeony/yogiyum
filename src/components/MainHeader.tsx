    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-4 sm:gap-6 text-[#e4573d]">
                <Link to="/LikedRestaurantsPage" className="hover:text-[#ff5630] hover:scale-120 transition-all duration-300 font-[Dongle] text-2xl" onClick={handleLikeClick}>
                    찜 리스트
                </Link>
...
function UserButton() {
        ...
        return (
            <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-white border border-[#ffd9cc] shadow-md rounded-md">
                    <DropdownMenuItem className="hover:bg-[#fff0eb] hover:pl-4 text-[#e4573d] transition-all duration-200" onClick={() => navigate("/myPage")}>
                        마이페이지
                    </DropdownMenuItem>
                    <DropdownMenuItem
                        onClick={async () => {
    ...
    return (
        <Link to="/login" className="text-[#e4573d] hover:text-[#ff7043] hover:bg-[#ffe9e3] px-2 py-1 rounded-md font-medium transition-all duration-300">
            로그인
        </Link>
    );
}
