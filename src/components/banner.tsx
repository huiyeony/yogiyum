            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="h-full w-1/3 flex items-center justify-center">
                        <img src={banners[index].image} alt="Banner" className="h-full w-full object-cover" />
                    </div>

                    {/* 오른쪽 텍스트 */}
                    <div className="flex-1 ml-6 flex items-center justify-between font-[jua]">
                        <motion.div whileHover={{ scale: 1.03 }} transition={{ type: "spring", stiffness: 200 }} className="text-black font-jua font-bold text-[20px]">
                            {banners[index].text}
                        </motion.div>

                        {/* 닫기 버튼 */}
                        <button onClick={() => setVisible(false)} className="text-black hover:text-gray-600 ml-4">
                            <X size={20} />
                        </button>
                    </div>
                </motion.div>
