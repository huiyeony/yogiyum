...
export default function RestaurantCard({ restaurant, rating, likedCount = 0, isLiked, onSearch }: Props) {
    ...
    return (
        <Card className="w-full sm:w-[calc(50%-0.75rem)] md:w-[calc(50%-0.4rem)] relative flex flex-col rounded-xl overflow-hidden bg-[#fffaf6] transition-transform duration-100 hover:scale-[1.02] p-3 gap-3">
            ...
            <div className="absolute top-1.5 right-1.5 z-10 flex flex-col items-center bg-white rounded-full shadow p-1">
                <Heart className="cursor-pointer" fill={isLiked ? "#ef4444" : "gray"} strokeWidth={0} size={20} onClick={likedButtonClick} />
                <span className="text-[10px] text-gray-500 mt-0.5">{likedCount}</span>
            </div>

            {/* 이미지 영역 */}
            <div className="relative w-full aspect-video bg-gray-100 mt-1">
                {!imageLoaded && <img src="/loading.gif" alt="로딩 중" className="absolute inset-0 w-full object-contain opacity-70" />}
                <img
                    src={restaurant.thumbnailUrl?.toString() || ""}
                    alt={restaurant.name}
                    onLoad={() => setImageLoaded(true)}
                    onError={() => setImageLoaded(true)}
                    className={`w-full h-40 object-cover transition-opacity duration-500 rounded-md ${imageLoaded ? "opacity-100" : "opacity-0"}`}
                />
            </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col">
                    <Link to={`/restaurant/${restaurant.id}`}>
                        <h2 className="text-lg font-semibold text-gray-800 hover:text-[#e4573d] hover:underline underline-offset-4 transition-colors duration-200">{restaurant.name}</h2>
                    </Link>
